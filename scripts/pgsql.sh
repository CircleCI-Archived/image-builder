...
apt-get update
apt-get install -qq postgresql-$POSTGRE_VERSION postgresql-contrib-$POSTGRES_VERSION
...
EOF
) >> /etc/postgresql/$POSTGRE_VERSION/main/postgresql.conf
...
EOF
) >> /etc/postgresql/$POSTGRE_VERSION/main/pg_hba.conf
